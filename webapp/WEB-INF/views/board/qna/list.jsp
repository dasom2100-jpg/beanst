    ...
    <div class="pagination">
        ...
        <c:choose>
            <c:when test="${pi.currentPage > 1}">
                <a href="${pageContext.request.contextPath}/qnaList.do?page=${pi.currentPage - 1}${queryKeyword}" class="page-prev" title="이전">&laquo;</a>
            </c:when>
            <c:otherwise>
        ...
        <c:forEach begin="${pi.startPage}" end="${pi.endPage}" var="p">
            <c:choose>
                ...
                <c:otherwise><a href="${pageContext.request.contextPath}/qnaList.do?page=${p}${queryKeyword}">${p}</a></c:otherwise>
            </c:choose>
        </c:forEach>
        <%-- 다음: 마지막 페이지면 비활성(링크 없음), 아니면 활성 링크 --%>
        <c:choose>
            <c:when test="${pi.currentPage < pi.maxPage}">
                <a href="${pageContext.request.contextPath}/qnaList.do?page=${pi.currentPage + 1}${queryKeyword}" class="page-next" title="다음">&raquo;</a>
            </c:when>
            <c:otherwise>
